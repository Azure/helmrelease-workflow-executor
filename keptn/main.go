...
import (
	...
	"github.com/Azure/orkestra-workflow-executor/keptn/pkg/packager"
...
func installAction(ctx context.Context, cancel context.CancelFunc, clientSet client.Client, config *rest.Config, hr *fluxhelmv2beta1.HelmRelease, interval time.Duration) {
	...
	// 0. Package up the HelmRelease as a helm chart
	helmPackager := packager.Helm()
	chartpath, err := helmPackager.Package(hr)
	defer func() {
		os.Remove(chartpath)
	}()

	if err != nil {
		log.Fatalf("helm packager got err: %v", err)
	}

	log.Infof("chart save at location : %s", chartpath)

	// A. Authenticate with Keptn Controller

	// B. Setup GIT repository if it doesnt already exist

	// C. Create keptn project if it doesnt already exist

	// D. Create the service (helmrelease name)

	// E. Add helmrelease helm chart to the keptn project.service

	// F. Trigger the shipyard.yaml stages
